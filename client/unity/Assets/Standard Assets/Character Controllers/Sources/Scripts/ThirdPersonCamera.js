...
//	}
	...
	gameObject.transform.position = targetCenter;
	gameObject.transform.position += /*currentRotation **/ Vector3.back * distance;

	// Set the height of the camera
	gameObject.transform.position.y = currentHeight;
...
function SetUpRotation (centerPos : Vector3, headPos : Vector3)
{
	...
	gameObject.transform.rotation = yRotation * Quaternion.LookRotation(relativeOffset);

	// Calculate the projected center position and top position in world space
	var centerRay = gameObject.transform.camera.ViewportPointToRay(Vector3(.5, 0.5, 1));
	var topRay = gameObject.transform.camera.ViewportPointToRay(Vector3(.5, clampHeadPositionScreenSpace, 1));
	...
	else
	{
		...
		gameObject.transform.rotation *= Quaternion.Euler(-extraLookAngle, 0, 0);
	}
}
